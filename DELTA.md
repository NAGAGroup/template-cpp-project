...
| D-20 | **LANDED** | The spdlog build-variant axis moves from `1.14`/`1.15` to `1.15`/`1.16`; feature and env rename `spdlog14` -> `spdlog16` | **Justification corrected 2026-08-09 — the original one was false and is retracted.** It is NOT true that our published `fmt` claims the name and makes other versions unreachable: a direct `fmt <11` resolves to 10.2.1 through the layer. The real rule is that priority fall-through applies to a DIRECT spec but not to a purely TRANSITIVE requirement — so spdlog 1.14's transitive `fmt >=11.0.1,<11.1` is refused while the same candidates are reachable directly. Measured: `fmt<11` alone SOLVES; `spdlog >=1.14,<1.15` alone FAILS; together they SOLVE; and the control (spdlog 1.14 + a direct `fmt >=11.2` that cannot help) FAILS. Promoting the spec cannot rescue the 1.14 cell, because the requirement lives in a source package's nested HOST solve which does not inherit env specs — fixing it there just pushes the run-export floor into every consuming env, and means a package pinning something it does not use. Both spdlog 1.16 builds want `fmt >=11.2`, which the top channel satisfies with our own wrapper, so 1.16 needs none of it. | Keep. Restoring 1.14 does not solve on this channel. |
