...
| D-28 | **LANDED** (toolchain gap workaround) | The static variant declares `binutils_linux-64` as a linux build-dependency. | AdaptiveCpp's activation exports the conda-forge archiver names but the toolchain does not SHIP them: an installed acpp env contains no archiver at all, so a static archive dies with exit 127 and `x86_64-conda-linux-gnu-ar: not found`. Shared libraries are unaffected — they go through the compiler driver — which is why only the static flavor failed. **THE DISTINCTION THAT MAKES THIS LEGITIMATE, because the same-looking fork has now decided three separate calls and it will come up again:** a package may state WHICH TOOLS ITS OWN BUILD NEEDS; a package may NOT reach into a dependency to steer what that dependency drags in. An archiver is the first — this package compiles and archives, so it declares an archiver. Pinning `fmt` in enginelib's host-deps to keep a spdlog cell on one ABI would have been the second, and was refused (D-23); the release lane's run-export is the second in a form we could not reach at all. The two look superficially identical — both are "add a dependency to make the build work" — and they are categorically different. If you are unsure which side you are on, ask whether the thing you are adding is a tool YOUR build invokes, or a version YOUR DEPENDENCY resolves. Third toolchain gap of the night, reported to ACPP alongside the run-export and the missing sanitizer runtimes — which share a theme with this one: the toolchain declaring what it does not deliver. | Delete when the activation stops exporting names it does not provide, or the toolchain ships them. |
...
**What does NOT belong in this section:** anything removed because it is not POSSIBLE. AM-2 is about breadth we declined to inherit, and it must not become the drawer that unavailable things get swept into — a capability gap and a design choice need to stay distinguishable, or the register stops being able to tell you which ones to revisit when the world changes. Capability gaps get their own row with a restore condition; D-26, the sanitizers, is the worked example.
