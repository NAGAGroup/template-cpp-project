...
- lock_file: --frozen (install from lock, ignore manifest sync) vs --locked (fail if out of sync); PIXI_FROZEN/PIXI_LOCKED env vars; satisfiability = envs+channels+packages matchspec-match lock (even timestamp/subdir); library-vs-app lock committing considerations; pixi-diff tooling.
