...
```
packages/enginelib/
├── pixi.toml              # workspace + DEFAULT package + version anchor
│                          #   + dev env + test envs + variant selector envs
...
is built *like the lib it tests*. Sanitizer/coverage variants are
linux-only (their envs' features are platform-scoped).
...
## Microarch subpackages (`v1`/`v3`/`v4`)
...
Why these are subpackages and not a build-variant axis — see
